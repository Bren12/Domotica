...
#define D5 14  // LED1 blinking
#define D6 12  // LED para sensor temperatura
#define D7 13  // Aquí va el pushbutton
#define D8 15  // LED para el pushbutton
...
// ----- Definiciones para el pushbutton
int val = 0;
...
const char* topico_salida = "TopicoEquipo5";
const char* topico_salida2 = "TopicoOutPhotoResitor";
const char* topico_entrada = "temp"; //Clima o aire acondicionado
char sTopicoEquipo5[50];
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish(topico_salida, sTopicoEquipo5);      
...
void conectarMQTT() {
 ...
 //   snprintf (msg, MSG_BUFFER_SIZE, "hello world #%ld", value);
    ...
    Serial.print(now); Serial.print("Publish message: "); Serial.println(sTopicoEquipo5);
    client.publish(topico_salida, sTopicoEquipo5);
...
void medirTemperatura() {
  ...
  //snprintf (sTopicoEquipo5, MSG_BUFFER_SIZE, "Temperatura(C) %5.2f, Humedad %5.2f #%ld",t,h,value);
}
...
// =================================================================
// ----- Funcion que detecta si el bóton ha sido oprimido
void push() {
  val = digitalRead(D7);
...
void setup() {
   ...
   // ----- Inicializacion pushbutton
   pinMode(D8,OUTPUT);
   pinMode(D7,INPUT);
   Serial.println("Iniciando Pushbutton");
}
...
void loop() {
   ...
   // Print en MQTT
   snprintf (sTopicoEquipo5, MSG_BUFFER_SIZE, "{\"t\":%5.2f,\"h\":%5.2f,\"v\":%d,\"d\":%5.2f,\"b\":%d,\"i\":%d}",t,h,x,d,val,i);
}
